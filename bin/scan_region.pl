...
if ($mce_flag or $evofold_flag or $tfbs_flag or $wgrna_flag or $segdup_flag or $anno_flag) {
	...
	if($refcds_flag and $refexon_flag){
		scanUCSCGene ($queryfile, $dbfile, 0, 'refcds,refexon', $kgxref, $name2_flag);
	} elsif ($knowngene_flag) {
...
sub scanUCSCGene {
	...
	my (%db_chr_loc);
	my ($name, $chr, $dbstrand, $start, $end, $cdsstart, $cdsend, $exoncount, $exonstart, $exonend, $start_status, $end_status);
	my ($gene_xref);
	...
	while (<DB>) {
		...
		} elsif ($dbtype eq 'refgene' or $dbtype eq 'refcds' or $dbtype eq 'refexon' or $dbtype eq 'refcds,refexon') {
			if (@record == 16) {
				($name, $chr, $dbstrand, $start, $end, $cdsstart, $cdsend, $exoncount, $exonstart, $exonend, $start_status, $end_status) = @record[1..10,13,14];		#human hg18, mouse
				$name2_flag and $name = $record[12];
				#next unless($start_status eq "cmpl" and $end_status eq "cmpl");
			} elsif (@record == 10) {
		...
		if ($dbtype eq 'knowngene' or $dbtype eq 'refgene') {
		...
		} elsif ($dbtype eq 'refcds,refexon'){
			$chr =~ s/^chr// or confess "Error: invalid chromosome designation (no 'chr' found) in dbfile $dbfile: <$_>\n";
			my @exonstart = split (/,/, $exonstart);
			my @exonend = split (/,/, $exonend);
			@exonstart == @exonend or confess "Error: invalid exon annotation in dbfile $dbfile (exonstart=@exonstart exonend=@exonend): <$_>\n";
			...
			for my $i (0 .. @exonstart-1) {
				if($cdsend == $cdsstart){
					1;
				} elsif($exonstart[$i] < $cdsstart and $exonend[$i] <= $cdsstart){
					next;
				} elsif($exonstart[$i] >= $cdsend and $exonend[$i] > $cdsend) {
					next;
				} elsif($exonstart[$i] < $cdsstart and $exonend[$i] > $cdsstart ) {
					$exonstart[$i] = $cdsstart;
				} elsif($exonstart[$i] < $cdsend and $exonend[$i] > $cdsend){
					$exonend[$i] = $cdsend;
				}
				$count_db_region++;
