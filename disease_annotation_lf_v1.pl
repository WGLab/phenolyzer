...
# Launch help or man
$help and pod2usage (-verbose=>1, -exitval=>1, -output=>\*STDERR);
$man and pod2usage (-verbose=>2, -exitval=>1, -output=>\*STDERR);
...
{
	##### 1. preprocessing $query_diseases #####
	####  1.1  the $query_diseases include all phenotype terms separated by tab or other characters, convert it to lower case and split into separate phrases

	my @disease_input = split (qr/[^ _,\w\.\-'\(\)\[\]\{\}:]+/, lc $query_diseases);
	
	####  1.2 remove heading and trailing spaces
	s/^\s+|\s+$// for @disease_input;		



	##### 2. Processing input terms #####
	...
	# results of the function were stored in files in @out_gene_scores_file

	##### 3. Merge gene_score files #####
	...
	print "DEBUG: open file for writing: MERGE: $out.merge_gene_scores\n" if $debug;
	print "DEBUG: open file for writing: ANNOTATED: $out.annotated_gene_scores\n" if $debug;
	...
	print "DEBUG: $seed_fh: $out.seed_gene_list\n" if $debug;
...
sub process_terms
{
	...
	#### if precalculated results were used ####
...
# Process one input at a time.
	...
	if ($debug)
	{
		print "DEBUG: results are stored in \@out_gene_scores_file:\n";
		print_array(@out_gene_scores_file);
	}

	## results of this function are stored in files in @out_gene_scores_file. 
}
...
sub process_individual_term
{
	...
	print "DEBUG: \$raw_individual_term = $raw_individual_term\n" if $debug;

	if ($individual_term =~ /^\W*$/){ return; } # next if there is no word 
		$individual_term=TextStandardize($individual_term);

	print "DEBUG: \$individual_term = $individual_term\n" if $debug;

	###### if input is 'all diseases' ######
...
#The probability of the gene when the disease is given
	...
	###### if input is NOT 'all diseases' (regular case) ######

	# For a normal term, disease name extension is needed
	# Expand the disease term and save them into files
	

	##### check if the input is an HPO ID #####
	my $input_is_hpo_id = 0;
	if ($raw_individual_term =~ /^hp:(\d+)/)
	{
		print STDERR "NOTICE: recognized HPO ID: $raw_individual_term\n";
		$input_is_hpo_id = 1;
	}
	my @diseases;
	my %disease_hash;

	###### if input is not an HPO ID ######
	# if input is an HPO ID, disease_extension is skipped

	if ($input_is_hpo_id == 0) 
	{
		print "DEBUG: running disease_extension\n" if $debug;
		my $diseases_reference = disease_extension($individual_term);
		my %disease_hash = %$diseases_reference;
		print "DEBUG: elements in \%disease_hash:\n" if $debug;
	...
	print STDERR "NOTICE: ", scalar (@diseases), " related diseases have been found by disease_extension\n" if $debug;

	# @diseases is an array of all related disease names found by disease_extension
	# An example of @diseases (input is benign_adult_familial_myoclonic_epilepsy): 
	...
	#	Epilepsy Myoclonic Benign Adult Familial 1
	#	Epilepsy Myoclonic Benign Adult Familial Type 1
	#	BAFME1
	#	Benign Adult Familial Myoclonic Epilepsy 1
	#	Cortical Myoclonic Tremor 
	#	CORTICAL MYOCLONIC TREMOR 
	#	Cortical Tremor Familial
	#	Epilepsy Familial Adult Myoclonic 1
	#	FAME1
	#	FCMTE1
	#	Epilepsy Myoclonic Benign Adult Familial 2
	...
	if ($is_phenotype) 
	{		
		# this is controlled by the --phenotype argument in command line; we will perform phenotype extension on the input phenotype, so @diseases array becomes larger
		# this section do two things: 
		#   1) find HPO IDs and store them in @hpo_ids;
		#   2) find HPO related disease and store them in %disease_score_hash

		#   An example of disease_score_hash:
		#   $disease_score_hash{androgen insensitivity syndrome}[0] = 0.33  (this is the score)
		#   $disease_score_hash{androgen insensitivity syndrome}[1] = androgen insensitivity syndrome (this is the disease name)

		#   An example of hpo_ids:
		#   HP:0008730

		($hash, @hpo_ids) = phenotype_extension($individual_term);
		%disease_score_hash = %$hash; ## disease scores get from phenotype_extension
		for (@diseases) {
			## remove disease that are already in @diseases
			my $disease_key = lc $_;
	...
	print STDERR "NOTICE: ", scalar (@hpo_ids), " related HPO terms have been found by phenotype_extension\n" if $debug;

	#The non-word characters are changed into '_'
	$individual_term=~s/\W+/_/g;
	print "DEBUG: formatted \$individual_term is $individual_term\n" if $debug;
	...
	### 1) output diseases that were found by disease_extension
	for (keys %disease_hash) 
	{
		...
		for my $line (@lines)
		{
			...
			print "DEBUG: diseases that were found by disease_extension: $out_line. \n" if $debug;
		}
	}

	### 2) output diseases that were found by phenotype_extension
	if ($is_phenotype) 
	{
		my $out_line;
		for (keys %disease_score_hash)
		{
			$out_line = join ("\t", ($disease_score_hash{$_}[1], $disease_score_hash{$_}[0]) )."\n";
			print "DEBUG: diseases that were found by phenotype_extension: $out_line. \n" if $debug;
			print OUT_DISEASE $out_line;
		}			
	}
	close (OUT_DISEASE);
	...
	if(@diseases == 0 and @hpo_ids == 0) # Li changed on 11/23/2019
	{
		print STDERR "ERROR: The input term -----$raw_individual_term------ has no corresponding names in the disease database, please check your spelling!!!\n";
		return;
	}
	...
	#### do TextStandardize on disease names ####
	@diseases = map {
		my @words = split("\t");
	...
	print_array(@diseases) if $debug; 

	##### score genes #####
	my ($item, $count) = score_genes (\@diseases, \@hpo_ids, $raw_individual_term);		#item is hash (key=gene value=[score, information])
	# Examples:
	# $item{GATA4}[0] = 0.1; $item{GATA4}[1] = HP:0008730 (HPO_PHENOTYPE_GENE)	Female external genitalia in individual with 46,XY karyotype	0.1
	
	my $item_size = %$item; # number of elements in %item
	my %output = ();
	@{$output{$_}} = @{$item->{$_}} for keys %$item;
	
	print "DEBUG: \$item_size = $item_size\n" if $debug;
	if($item_size == 0){ # Li changed on 11/23/2019
		print STDERR "ERROR: The input term -----$individual_term------ has no results!!!\n";
		return;
	}

	my $gene_scores_out_file = "$out"."_$individual_term"."_gene_scores";
	print STDERR "NOTICE: Writing to file: $gene_scores_out_file\n";

	open(OUT_GENE_SCORE, ">$gene_scores_out_file") or die "cannot write to $gene_scores_out_file
	\n";
	print OUT_GENE_SCORE "Tuple number in the gene_disease database for the term $individual_term: $item_size\n";
...
#The probability of the gene when the disease is given
	...
	print STDERR "NOTICE: finished processing term: $raw_individual_term\n";
	print STDERR "----------------------------------------------------------------------\n";
	close (OUT_GENE_SCORE);
}
...
sub disease_extension{
	...
	my $input_term=$_[0];
	print "DEBUG: running disease_extension, \$input_term = $input_term\n" if $debug;
	print STDERR "The journey to find all related disease names of your query starts!\n";
	$input_term =~ s/[\W_]+/ /g;

	print "DEBUG: running disease_extension, formatted \$input_term = $input_term\n" if $debug;
	...
	print STDERR "The exact match (case non-sensitive) was used for disease/phenotype name match!! \n" if($if_exact_match);

	#### searching matched items in DB_COMPILED_OMIM_ID_DISEASE ####

	print "DEBUG: reading file: $path/$omim_disease_id_file\n" if $debug;
	...
	close OMIM_DISEASE_ID;

	my $size = %disease_extend;
	print STDERR "NOTICE: $size matched item(s) have been found in the OMIM database!\n";

	#### searching matched items in DB_COMPILED_DISEASE_COUNT ####
	$input_term = lc $input_term;

	#Query disease in the compiled list from gene_disease relations
	print "DEBUG: reading file: $path/$disease_count_file\n" if $debug;
...
#Save the disease name into hash, key is the disease name in lower case form withougt "-"
	...
	$size = %disease_extend - $size;
	print STDERR "NOTICE: $size matched item(s) have been found in the gene disease databases!\n";
	
	#### searching matched items in DB_COMPILED_CTD_DISEASES ####
	print "DEBUG: reading file: $path/$ctd_disease_file\n" if $debug;
	...
	$size = %disease_extend - $size;
	print STDERR "NOTICE: $size matched item(s) have been found in the CTD (Medic) databases!\n";
...
#Second find all children of the terms found in the first round
	...
	print STDERR "The descendants search in the CTD (Medic) databases has been done! \n";
	...
	if (-f "$work_path/ontology_search.pl") {
		...
		print STDERR "The descendants search in disease_ontology (DO) database has been done! \n";
	} else {
		print STDERR "ERROR: The $work_path/ontology_search.pl file couldn't be found, so the disease_ontology (DO) database won't be used! \n";
	}
	...
	if ($debug)
	{
		for (keys %disease_extend)
		{
			print "DEBUG: \$disease_extend{$_} = $disease_extend{$_}\n";
		}
	}
...
sub phenotype_extension{
	...
	# $disease_hash{androgen insensitivity syndrome}[0] = 0.33 (score)
	# $disease_hash{androgen insensitivity syndrome}[1] = androgen insensitivity syndrome (disease name)

	print "DEBUG: Start of phenotype_extension. line number: ", __LINE__, "\n"  if $debug;
	...
	print "DEBUG: \$raw_input_term = $raw_input_term\n" if $debug;
	$input_term =~s/[\W_]+/ /g;

	print "DEBUG: \$input_term = $raw_input_term\n" if $debug;
	my %disease_hash; ## one of the return variable
	my @hpo_ids;      ## one of the return variable

	# %disease_hash( "disease_name_key" => [score, original_disease_name] )
	##### check if files exist #####
	if( not -f "$work_path/ontology_search.pl" ) {
		print STDERR "NOTICE: The $work_path/ontology_search.pl file couldn't be found! \n";
		exit;
	}
	...
	if ( not -f "$path/$omim_description_file" )
	{
		print STDERR "ERROR: The $omim_description_file couldn't be found!!! The phenotype_annotation couldn't be conducted properly!! \n";
		exit;
	}
	
	##### get HPO IDs from hpo.obo file. child HPO IDs may be also found #####
	
	my $line = `perl $work_path/ontology_search.pl -o $path/hpo.obo -format id -p '$input_term' `;
	print STDERR "executing ontology_search.pl on '$input_term' to expand phenotype terms\n" if $debug;
	
	@hpo_ids = split("\n", $line);

	print "DEBUG: elements in \@hpo_ids:\n" if $debug;
	print_array(@hpo_ids) if $debug;
	print STDERR "NOTICE: Found ", scalar (@hpo_ids), " additional phenotype terms\n";

	##### get HPO IDs and disease names from DB_HPO_ANNOTATION #####
	print "DEBUG: reading file: $path/$hpo_annotation_file\n" if $debug;
	open (HPO_ANNOTATION, "$path/$hpo_annotation_file") or die "ERROR: Can't open $hpo_annotation_file!!! \n";
	my @hpo_annotation = <HPO_ANNOTATION>;
	close HPO_ANNOTATION;
	...
	while($i<@hpo_ids and $j<@hpo_annotation)
	{
		...
		if($hpo_ids[$i] eq "HP:".$words[0]) {
			print "\$hpo_annotation[\$j] = $hpo_annotation[$j]\n" if $debug;

			my @diseases = split(";",$words[2]);
			print "DEBUG: elements in \@diseases: (line number is ", __LINE__, ")\n" if $debug;
			print_array(@diseases) if $debug;
			...
			for my $individual_disease(@diseases) 
			{
				...
				### get score of each $individual_disease_key
				$disease_hash{$individual_disease_key}[0] = 0; ## score
				$disease_hash{$individual_disease_key}[1] =lc $individual_disease if not $disease_hash{$individual_disease_key}[1]; ## $individual_disease
	...
	print "DEBUG: End of phenotype_extension from HPO_ANNOTATION. line number: ", __LINE__, "\n" if $debug;
	print "DEBUG: elements in \@hpo_ids:\n" if $debug;
	print_array(@hpo_ids) if $debug;
	print "DEBUG: elements in \%disease_hash:\n" if $debug;
	...
	if ($debug)
	{
		my $a;
		my $b;
		while ( ($a, $b) = each %disease_hash ) 
		{
	    	print "\$disease_hash{$a}[0] = $disease_hash{$a}[0]\n";
	    	print "\$disease_hash{$a}[1] = $disease_hash{$a}[1]\n";
		}
	}

	##### get HPO IDs and disease names from DB_OMIM_DESCRIPTION #####
	print "DEBUG: reading file: $path/$omim_description_file\n" if $debug;
...
{
	### This is the JAX HPO database (DB_COMPILED_HPO_PHENOTYPE_GENE)
	print "DEBUG: reading file: ${path}/$phenotype_to_gene_file\n" if $debug;
...
sub score_genes{
	...
	### score_genes (\@diseases, \@hpo_ids, $raw_individual_term);
	...
	##### Get info from JAX HPO to gene database(DB_COMPILED_HPO_PHENOTYPE_GENE) #####
	my $hpo_id_to_gene_hash_ref = get_phenotype_to_gene_hash();
	my %hpo_id_to_gene_hash = %$hpo_id_to_gene_hash_ref;
	# in %hpo_id_to_gene_hash, key is HPO ID, value is a comma separated Gene name list. 
	# Example: $hpo_id_to_gene_hash{HP:0008730} = GATA4,WT1,SOX9,POR,AR,CYP11A1,SRY,CYP17A1,NR5A1,HSD3B2,MAP3K1,ZFPM2,CYB5A,DMRT3,WWOX,VAMP7,NR0B1

	if ($debug)
	{
		my $h = "HP:0008730";
		print "\$hpo_id_to_gene_hash{$h} = $hpo_id_to_gene_hash{$h}\n";
	}

	my $hpo_id_to_name_hash_ref = get_hpo_id_to_name_hash();
	my %hpo_id_to_name_hash = %$hpo_id_to_name_hash_ref;
	# in %hpo_id_to_name_hash, key is HPO ID, value is HPO name. 
	# Example: $hpo_id_to_name_hash{HP:0008730} = Female external genitalia in individual with 46,XY karyotype
	if ($debug)
	{
		my $h = "HP:0008730";
		print "\$hpo_id_to_name_hash{$h} = $hpo_id_to_name_hash{$h}\n";
	}

	#### get mapping from gene name to HPO ID ####
	my %jax_phenotype_related_genes_hash; # key = gene name, value = hpo_id 

	foreach (@hpo_ids)
	{
		my $hpo_id = $_;
		if (not exists($hpo_id_to_gene_hash{$hpo_id})){
            print STDERR "WARNING!: $hpo_id not exists in the DB_COMPILED_HPO_PHENOTYPE_GENE file!\n";
			next;
		}
		my @gene_array = split(",", $hpo_id_to_gene_hash{$hpo_id});	
		foreach (@gene_array) {
			$jax_phenotype_related_genes_hash{$_} = $hpo_id;
		}
	}

	if ($debug)
	{
		print "DEBUG: genes in \@jax_phenotype_related_genes_hash:\n";
		foreach (keys %jax_phenotype_related_genes_hash) 
		{
			print "$_ => $jax_phenotype_related_genes_hash{$_}\n";
		}
	}
	...
	#### read DB_COMPILED_GENE_DISEASE_SCORE ####
	print "DEBUG: open file: ${path}/$gene_disease_score_file\n" if $debug;
	...
	print "number of elements in \@disease_gene_score is ", scalar (@disease_gene_score), "\n" if ($debug);

	#### read addon databases ####
	# the addon database should have the same format as DB_COMPILED_GENE_DISEASE_SCORE
	my @addon_disease_gene_score;
	if($addon_gene_disease_score_file)
	{
		print "DEBUG: \$addon_gene_disease_score_file = $addon_gene_disease_score_file\n" if $debug;
	...
	#### sorting @disease_gene_score by disease name and formatting ####
	# GENE	DISEASE	DISEASE_ID	SCORE	SOURCE
	# example: FAM111A,GCLEB,KCS2	Gracile bone dysplasia	OMIM:602361	0.0625	CLINVAR
	...
	print_array(@diseases ) if $debug;
	# example of diseases[0]: androgen insensitivity syndrome	0.33
	...
	for (keys %jax_phenotype_related_genes_hash)
	{
		my $gene = $_;
		my $hpo_id = $jax_phenotype_related_genes_hash{$gene};
		...
		if ( exists($item{$gene}) )
		{
			...
			# 1.0 was hard coded in the above line.
		}else{
			$item{$gene}[0] = $HPO_GENE_WEIGHT; 
			$item{$gene}[1] = "$hpo_id (HPO_PHENOTYPE_GENE)\t$hpo_name\t1.0\n";
	...
	if ($debug)
	{
		for (keys %item)
		{
			my $key = $_;
			my $value = $item{$key};
			print "DEBUG: after adding jax db: \$item{$key}[0] = $item{$key}[0]; \$item{$key}[1] = $item{$key}[1]\n";
		}
	}

	my @value_array;

	print STDERR "NOTICE: The gene score process has been done for term: $individual_term!!\n";
...
sub merge_result {
	...
	for my $filename(@out_gene_scores_file) {
		if($filename=~/${out}_(\w+)_gene_scores$/){
			...
			print "NOTICE: reading file: $filename\n";
			...
			while (my $line = <GENE_SCORE>) 
			{
				...
				# get count from the first line. Example: 
				# Tuple number in the gene_disease database for the term benign_adult_familial_myoclonic_epilepsy: 3
				# in this case, $individual_count = 3; $count += 3
	...
	if ($debug)
	{
		for (keys %item)
		{
			print "DEBUG: merged score: \$item{$_}[0] = $item{$_}[0]; \$item{$_}[1] = $item{$_}[1]\n";
		}
	}
	return (\%item,$count);
}
...
#10      NAT2    AAC2|NAT-2|PNAT
	print "DEBUG: \$path/\$gene_annotation_file = $path/$gene_annotation_file\n" if $debug;
...
# Predict genes based on Addon Gene-Gene relations
	...
	print STDERR "NOTICE: Loading HPRD Database ... ";
	open (HPRD, "$path/$hprd_file") or die "Can't open $path/$hprd_file !";
#Predict genes based on Human Protein Interactions
	...
	print STDERR "NOTICE: Loading HTRI Database ... ";
...
#Predict genes based on gene family
	open (GENE_FAMILY, "$path/$gene_family_file") or die "Can't open $path/$gene_family_file!";
	print STDERR "NOTICE: Loading HGNC Gene Family Database ... ";
	$i = 0;
	my %in_gene_family = ();
...
#Each related gene in the gene_family
	...
	print STDERR "NOTICE: Loading Biosystem Database ... ";
	open (BIOSYSTEM, "$path/$biosystem_file") or die "Can't open $path/$biosystem_file !";
