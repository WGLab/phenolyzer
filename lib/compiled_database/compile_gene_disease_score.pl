use strict;
use Carp;
open(OMIM,"DB_OMIM_GENE_DISEASE") or die ;
open(CLINVAR,"DB_CLINVAR_GENE_DISEASE") or die ;
open(ORPHANET,"DB_ORPHANET_GENE_DISEASE") or die ;
open(GENEREVIEWS,"DB_COMPILED_GENEREVIEWS") or die ;
open(GWAS, "DB_GWAS_GENE_DISEASE") or die ;

open(OUT,">DB_COMPILED_GENE_DISEASE_SCORE") or die ;
